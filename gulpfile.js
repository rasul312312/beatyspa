...
const gulp         = require('gulp'),                       // Сам сборщик Gulp
      ...
      sasslibs = require('gulp-sass'),                  // Пакет компиляции SASS/SCSS
...
gulp.task('sasslibs', () => {
    return gulp.src(`${dev}/sass/libs.sass`)   // В этом файле(-ах) хранятся основные стили, остальные следует импортировать в него
        .pipe(sourcemaps.init())                    // инциализация sourcemap'ов
        .pipe(sass({
            outputStyle: ':nested'                  // компиляции в CSS с отступами
        }))
...
gulp.task('default', ['sass', 'sasslibs', 'img', 'pug', 'jsLibs', 'scripts', 'browser-sync'], () => {
    // стили
    gulp.watch([`${dev}/sass/style.sass`, `${dev}/sass/assets/*.sass`, `${dev}/sass/componets/*.sass`, `${dev}/libs/**/*.css`], ['sass']);
    gulp.watch([`${dev}/sass/libs.sass`], ['sasslibs']);
    // разметка
...
// Собираем наш билд
gulp.task('build', ['clean', 'img', '_sass', '_sasslibs', '_pug', 'jsLibs', '_scripts', 'service-worker'], () => {
